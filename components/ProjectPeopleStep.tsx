...
export const ProjectPeopleStep: React.FC<{ project: Project; onDone: () => void }> = ({ project, onDone }) => {
  ...
  return (
    <div className="h-[100dvh] w-full flex justify-center bg-zinc-100">
      <div className="w-full max-w-md h-full flex flex-col bg-zinc-50 shadow-sm">
        <header className="px-5 pt-7 pb-2 shrink-0 text-center">
          <div className="text-3xl mb-1">{projectEmoji(project.type)}</div>
          <h1 className="text-xl font-extrabold tracking-tight text-zinc-900">Invita a tu gente</h1>
          <p className="text-sm text-zinc-500 mt-1">«{project.name}» está listo. Suma a tus panas.</p>
        </header>

        <div className="flex-1 overflow-y-auto px-5 pt-2 pb-4 no-scrollbar">
          {loading ? (
            <div className="flex justify-center py-10"><Loader2 className="animate-spin text-zinc-300" size={24} /></div>
          ) : (
            <InvitePanel project={project} participants={participants} onAdded={p => setParticipants(prev => [...prev, p])} />
          )}
        </div>

        <div className="p-5 pt-2 border-t border-zinc-100 shrink-0">
