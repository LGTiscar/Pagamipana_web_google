...
// La home usa el resumen (ProjectOverview); al abrir construimos un Project.
...
export const HomeProjects: React.FC<Props> = ({ auth, onOpenProject, onQuickSplit }) => {
  ...
  return (
    ...
    <div className="w-full max-w-md h-full flex flex-col bg-zinc-50 relative shadow-sm">
      {/* Header */}
          ...
          <div className="relative">
            ...
            {showMenu && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-60 bg-white border border-zinc-200 rounded-2xl shadow-xl z-20 overflow-hidden">
                  <div className="px-4 py-3 border-b border-zinc-100">
                    <div className="text-[11px] text-zinc-400 font-semibold">Sesión iniciada</div>
                    <div className="text-sm font-bold text-zinc-900 truncate">{auth.user?.email ?? 'Cuenta'}</div>
                  </div>
                  <button onClick={async () => { setShowMenu(false); await auth.signOut(); }} className="w-full flex items-center gap-2 px-4 py-3 text-sm font-semibold text-red-600 hover:bg-red-50">
      ...
      {auth.authError && (
        <div className="mx-5 mb-3 px-4 py-2.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-sm font-medium shrink-0">
          {auth.authError}
        </div>
      )}
            ...
            {projects.map(o => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left bg-white border border-zinc-200 rounded-2xl p-4 hover:border-zinc-300 transition-all active:scale-[0.99]"
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-11 h-11 rounded-xl bg-zinc-50 border border-zinc-100 flex items-center justify-center text-xl shrink-0">{projectEmoji(o.type)}</div>
                      <div className="min-w-0">
                        <div className="font-bold text-zinc-900 truncate">{o.name}</div>
                        <div className="text-xs text-zinc-400 mt-0.5">{o.member_count} {o.member_count === 1 ? 'persona' : 'personas'}</div>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-[11px] font-semibold text-zinc-400">{pos ? 'Te deben' : neg ? 'Le debes' : 'En paz'}</div>
                      <div className={`text-base font-extrabold tabular-nums ${pos ? 'text-blue-700' : neg ? 'text-red-500' : 'text-zinc-400'}`}>
                  ...
                  {o.avatars.length > 0 && (
                    <div className="flex -space-x-2 mt-3">
                      {o.avatars.slice(0, 5).map((a, i) => (
                        <span key={i} className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold ring-2 ring-white ${a.color ?? 'bg-zinc-200 text-zinc-700'}`}>{initials(a.name)}</span>
                      ))}
                    </div>
                  )}
                </button>
